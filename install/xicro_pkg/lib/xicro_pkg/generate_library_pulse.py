...
def get_params(qq):
    try:
        q=qq
        path = os.path.join(get_package_share_directory('xicro_pkg'),'config', 'setup_xicro_pulse.yaml')
...
def create_hstruct(fw):
    ...
    fw.write("\r        void set_callback(int idx, void (funct)())")
    fw.write("\r        {")
    fw.write("\r           twtArray[idx].funct = funct;")
    fw.write("\r        }")

    fw.write("\r\r        struct tst")
    fw.write("\r        {")
    fw.write("\r            void (*funct)();")
    fw.write("\r        };")

    # fw.write("\r\r        tst twtArray[" + len(id_topic) + "];")
    fw.write("\r\r        tst twtArray[1];\r\r")
            ...
            if(line=="!#@ gen_void\n"): 
                try:
                    ...
                    for i in range(0,len(listVoid)):
                        ...
                        fw.write("    _SendUint32((uint32_t*)&Publisher_pulsa.message.data,1);\n")
                        for j in range(0,len(dataType[i])):
